...
var Transaction = onixcore.Transaction;
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/onix.conf')));
var OnixService = proxyquire('../../lib/services/onixd', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultOnixConf = fs.readFileSync(path.resolve(__dirname, '../data/default.onix.conf'), 'utf8');

describe('Onix Service', function() {
	...
	var baseConfig = {
		node: {
			network: onixcore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		it('will create an instance', function() {
			var onixd = new OnixService(baseConfig);
			should.exist(onixd);
		});
		it('will create an instance without `new`', function() {
			var onixd = OnixService(baseConfig);
			should.exist(onixd);
		});
		it('will init caches', function() {
			var onixd = new OnixService(baseConfig);

			should.exist(onixd.utxosCache);
			should.exist(onixd.txidsCache);
			should.exist(onixd.balanceCache);
			should.exist(onixd.summaryCache);
			should.exist(onixd.transactionDetailedCache);
			should.exist(onixd.accountInfo);

			should.exist(onixd.transactionCache);
			should.exist(onixd.rawTransactionCache);
			should.exist(onixd.rawJsonTransactionCache);
			should.exist(onixd.blockCache);
			should.exist(onixd.blockJsonCache);
			should.exist(onixd.blockSubsidyCache);
			should.exist(onixd.rawBlockCache);
			should.exist(onixd.blockHeaderCache);
			should.exist(onixd.zmqKnownTransactions);
			should.exist(onixd.zmqKnownBlocks);
			should.exist(onixd.lastTip);
			should.exist(onixd.lastTipTimeout);
			should.equal(onixd.dgpInfoCache, null, 'should be null');
			should.equal(onixd.miningInfoCache, null, 'should be null');
			should.equal(onixd.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(onixd.maxTxids, 1000);
			should.equal(onixd.maxTransactionHistory, 50);
			should.equal(onixd.maxAddressesQuery, 10000);
			should.equal(onixd.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(onixd.spawnRestartTime, 5000);
			should.equal(onixd.spawnStopTime, 10000);

			// try all interval
			should.equal(onixd.tryAllInterval, 1000);
			should.equal(onixd.startRetryInterval, 5000);

			// rpc limits
			should.equal(onixd.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(onixd.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var onixd = new OnixService(baseConfig);
			onixd.nodes.should.deep.equal([]);
			onixd.nodesIndex.should.equal(0);
			onixd.nodes.push({ client: sinon.stub() });
			should.exist(onixd.client);
		});
		it('will set subscriptions', function() {
			var onixd = new OnixService(baseConfig);
			onixd.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var onixd = new OnixService(baseConfig);
			onixd._initDefaults({ transactionConcurrency: 10 });
			onixd.transactionConcurrency.should.equal(10);
			onixd._initDefaults({});
			onixd.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			OnixService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var onixd = new OnixService(baseConfig);
			var methods = onixd.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var onixd = new OnixService(baseConfig);
			var events = onixd.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('onixd/rawtransaction');
			events[0].scope.should.equal(onixd);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('onixd/hashblock');
			events[1].scope.should.equal(onixd);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('onixd/addresstxid');
			events[2].scope.should.equal(onixd);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('onixd/addressbalance');
			events[3].scope.should.equal(onixd);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var onixd = new OnixService(baseConfig);
			onixd.subscribe = sinon.stub();
			onixd.unsubscribe = sinon.stub();
			var events = onixd.getPublishEvents();

			events[0].subscribe('test');
			onixd.subscribe.args[0][0].should.equal('rawtransaction');
			onixd.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			onixd.unsubscribe.args[0][0].should.equal('rawtransaction');
			onixd.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			onixd.subscribe.args[1][0].should.equal('hashblock');
			onixd.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			onixd.unsubscribe.args[1][0].should.equal('hashblock');
			onixd.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var onixd = new OnixService(baseConfig);
			var emitter = {};
			onixd.subscribe('hashblock', emitter);
			onixd.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			onixd.subscribe('rawtransaction', emitter2);
			onixd.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			onixd.subscribe('hashblock', emitter1);
			onixd.subscribe('hashblock', emitter2);
			onixd.subscribe('hashblock', emitter3);
			onixd.subscribe('hashblock', emitter4);
			onixd.subscribe('hashblock', emitter5);
			onixd.subscriptions.hashblock.length.should.equal(5);

			onixd.unsubscribe('hashblock', emitter3);
			onixd.subscriptions.hashblock.length.should.equal(4);
			onixd.subscriptions.hashblock[0].should.equal(emitter1);
			onixd.subscriptions.hashblock[1].should.equal(emitter2);
			onixd.subscriptions.hashblock[2].should.equal(emitter4);
			onixd.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var onixd = new OnixService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			onixd.subscriptions.hashblock = [emitter1];
			onixd.unsubscribe('hashblock', emitter3);
			onixd.subscriptions.hashblock.length.should.equal(1);
			onixd.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			onixd.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(onixd.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var onixd = new OnixService(baseConfig);
			var emitter = new EventEmitter();
			onixd.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var onixd = new OnixService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			onixd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			onixd.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			onixd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			onixd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			onixd.unsubscribeAddress(emitter1);
			onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			onixd.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			onixd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			onixd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			onixd.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			onixd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(onixd, 'unsubscribeAddressAll');
			onixd.unsubscribeAddress(emitter1);
			onixd.unsubscribeAddressAll.callCount.should.equal(1);
			onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			onixd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			onixd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			onixd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			onixd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			onixd.unsubscribeAddressAll(emitter1);
			onixd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			onixd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			onixd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(onixd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var onixd = new OnixService(baseConfig);
			var config = onixd._getDefaultConfig();
			config.should.equal(defaultOnixConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a onix.conf file', function() {
			var TestOnix = proxyquire('../../lib/services/onixd', {
				fs: {
					readFileSync: readFileSync,
			...
			var onixd = new TestOnix(baseConfig);
			onixd.options.spawn.datadir = '/tmp/.onix';

			sinon.spy(onixd, '_expandRelativeDatadir');
			sinon.spy(onixd, '_checkConfigIndexes');
			sinon.spy(onixd, '_getDefaultConf')
			sinon.spy(onixd, '_getNetworkConfigPath');
			sinon.spy(onixd, '_parseBitcoinConf');

			var node = {};
			onixd._loadSpawnConfiguration(node);

			onixd._expandRelativeDatadir.callCount.should.equal(1);
			onixd._checkConfigIndexes.callCount.should.equal(1);
			onixd._getDefaultConf.callCount.should.equal(1);
			onixd._getNetworkConfigPath.callCount.should.equal(1);
			onixd._parseBitcoinConf.callCount.should.equal(2);

			should.exist(onixd.spawn.config);
			onixd.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: onixcore.Networks.testnet,
					configPath: '/tmp/.onixcore/onixcore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			onixd.options.spawn.datadir = './data';
			var node = {};
			onixd._loadSpawnConfiguration(node);
			...
			onixd.options.spawn.datadir.should.equal('/tmp/.onixcore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var TestOnix = proxyquire('../../lib/services/onixd', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badonix.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var onixd = new TestOnix(baseConfig);
			(function() {
				onixd._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(onixcore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the onix.conf file', function() {
			var onixd = new OnixService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				onixd._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the onix.conf file', function() {
			var onixd = new OnixService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the onix.conf file', function() {
			var onixd = new OnixService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the onix.conf file', function() {
			var onixd = new OnixService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the onix.conf file', function() {
			var onixd = new OnixService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				onixd.transactionDetailedCache.set(keys[i], {});
				onixd.utxosCache.set(keys[i], {});
				onixd.accountInfo.set(keys[i], {});
				onixd.txidsCache.set(keys[i], {});
				onixd.balanceCache.set(keys[i], {});
				onixd.summaryCache.set(keys[i], {});
				onixd.blockOverviewCache.set(keys[i], {});
				onixd.blockJsonCache.set(keys[i], {});
			}
			onixd._resetCaches();
			should.equal(onixd.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(onixd.utxosCache.get(keys[0]), undefined);
			should.equal(onixd.accountInfo.get(keys[0]), undefined);
			should.equal(onixd.txidsCache.get(keys[0]), undefined);
			should.equal(onixd.balanceCache.get(keys[0]), undefined);
			should.equal(onixd.summaryCache.get(keys[0]), undefined);
			should.equal(onixd.blockOverviewCache.get(keys[0]), undefined);
			should.equal(onixd.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			...
			onixd._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				onixd.nodes[0].client.getInfo.callCount.should.equal(1);
				onixd.nodes[1].client.getInfo.callCount.should.equal(1);
				onixd.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			onixd._tryAllClients(function(client, next) {
				...
				onixd.nodes[1].client.getInfo.callCount.should.equal(1);
				onixd.nodes[2].client.getInfo.callCount.should.equal(1);
				onixd.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert onixd-rpc error object into JavaScript error', function() {
			var onixd = new OnixService(baseConfig);
			var error = onixd._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			onixd.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			onixd.nodes.push({
				client: {
			...
			onixd._initChain(function() {
				log.info.callCount.should.equal(1);
				onixd.getRawBlock.callCount.should.equal(1);
				onixd.getRawBlock.args[0][0].should.equal('genesishash');
				onixd.height.should.equal(5000);
				onixd.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			onixd.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			onixd._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			...
			baseConfig.node.network = onixcore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: onixcore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
			...
			var onixd = new OnixService(config);
			onixd._getDefaultConf().rpcport.should.equal(8332);
		});
		it('will get default rpc port for testnet', function() {
			...
			var onixd = new OnixService(config);
			onixd._getDefaultConf().rpcport.should.equal(18332);
		});
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var onixd = new OnixService(config);
			should.equal(onixd._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var onixd = new OnixService(config);
			onixd._getNetworkConfigPath().should.equal('testnet3/onix.conf');
		});
		it('will get default rpc port for regtest', function() {
			onixcore.Networks.enableRegtest();
			var config = {
			...
			var onixd = new OnixService(config);
			onixd._getNetworkConfigPath().should.equal('regtest/onix.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var onixd = new OnixService(baseConfig);
			onixd.node.network = onixcore.Networks.testnet;
			onixd._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var onixd = new OnixService(baseConfig);
			onixd.node.network = onixcore.Networks.testnet;
			onixcore.Networks.enableRegtest();
			onixd._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var onixd = new OnixService(baseConfig);
			onixd.node.network = onixcore.Networks.livenet;
			onixcore.Networks.enableRegtest();
			should.equal(onixd._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			onixd._rapidProtectedUpdateTip = sinon.stub();
			onixd.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			onixd._zmqBlockHandler(node, message);
			onixd._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			var message = new Buffer('76843f1dfe455347d897e757ff5ff51a16a0f62b5d02a659c5680392de3a2b89', 'hex');
			onixd._rapidProtectedUpdateTip = sinon.stub();
			sinon.spy(onixd, 'emit');
			onixd.on('block', function(block) {
				block.should.equal(message);
				onixd.emit.callCount.should.equal(1);
				onixd._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			onixd._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			onixd._zmqBlockHandler(node, message);
			onixd._rapidProtectedUpdateTip.callCount.should.equal(1);
			onixd._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			onixd._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(onixd, 'emit');
			var emitter = new EventEmitter();
			onixd.subscriptions.hashblock.push(emitter);
			emitter.on('onixd/hashblock', function(blockHash) {
				onixd._rapidProtectedUpdateTip.callCount.should.equal(1);
				onixd.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			...
			var count = 0;
			function repeat() {
				onixd._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd.syncPercentage = sinon.stub();
			sinon.spy(onixd, '_resetCaches');
			onixd.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				onixd._resetCaches.callCount.should.equal(1);
			...
			onixd._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			onixd.on('synced', function() {
				onixd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			onixd.on('synced', function() {
				onixd.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			onixd.on('error', function(err) {
				log.error.callCount.should.equal(1);
				onixd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd.syncPercentage = sinon.stub();
			onixd._resetCaches = sinon.stub();
			onixd.on('tip', function(height) {
				onixd._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				onixd.height.should.equal(10);
				done();
			});
			var node = {
				client: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using onixcore.Transaction', function() {
			var onixd = new OnixService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = onixcore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(onixcore.Networks.testnet);
			var outputAddress = onixcore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = onixcore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: onixcore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = onixd._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			...
			tx.addOutput(onixcore.Transaction.Output({
				script: onixcore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = onixd._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var onixd = new OnixService(baseConfig);
			var tx = onixcore.Transaction();
			var address = onixcore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(onixcore.Transaction.Output({
				script: onixcore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(onixcore.Transaction.Output({
				script: onixcore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			onixd._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			onixd.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('onixd/addresstxid', function(data) {
			...
			onixd._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			onixd.subscriptions.rawtransaction.push(emitter);
			emitter.on('onixd/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			onixd._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(onixd, 'emit');
			onixd.subscriptions.rawtransaction.push(emitter);
			emitter.on('onixd/rawtransaction', function() {
				done();
			});
			var node = {};
			onixd._zmqTransactionHandler(node, expectedBuffer);
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd._updateTip = sinon.stub();
			onixd._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			onixd.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
			...
			onixd._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				onixd._updateTip.callCount.should.equal(11);
				onixd._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		it('will call subscribe on zmq socket', function() {
			...
			onixd._subscribeZmqEvents(node);
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				onixd._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			onixd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				onixd._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			onixd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				onixd._zmqBlockHandler.callCount.should.equal(0);
				onixd._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var onixd = new OnixService(baseConfig);
			var node = {};
			onixd._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			onixd._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			onixd._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			onixd._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestOnixService = proxyquire('../../lib/services/onixd', {
				fs: {
					readFile: readFile
				}
			});
			var onixd = new TestOnixService(baseConfig);
			onixd.spawnStopTime = 1;
			onixd._process = {};
			onixd._process.kill = sinon.stub();
			onixd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				onixd._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			onixd._process.kill = sinon.stub().throws(error2);
			onixd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd._loadSpawnConfiguration = sinon.stub();
			onixd._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			onixd._spawnChildProcess(function(err) {
				onixd._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var onixd = new OnixService(baseConfig);
			onixd._loadSpawnConfiguration = sinon.stub();
			onixd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			onixd._spawnChildProcess(function(err) {
				onixd._loadSpawnConfiguration.callCount.should.equal(1);
				onixd._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var onixd = new TestOnixService(config);
			onixd.spawn = {};
			onixd._loadSpawnConfiguration = sinon.stub();
			onixd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			onixd.node.stopping = true;
			onixd._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			onixd._loadSpawnConfiguration = sinon.stub();
			onixd.spawn = {};
			onixd.spawn.exec = 'testexec';
			onixd.spawn.configPath = 'testdir/onix.conf';
			onixd.spawn.datadir = 'testdir';
			onixd.spawn.config = {};
			onixd.spawn.config.rpcport = 20001;
			onixd.spawn.config.rpcuser = 'onix';
			onixd.spawn.config.rpcpassword = 'password';
			onixd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			onixd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			onixd._initZmqSubSocket = sinon.stub();
			onixd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			onixd._checkReindex = sinon.stub().callsArgWith(1, null);
			onixd._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/onix.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				onixd._loadTipFromNode.callCount.should.equal(1);
				onixd._initZmqSubSocket.callCount.should.equal(1);
				should.exist(onixd._initZmqSubSocket.args[0][0].client);
				onixd._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				onixd._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(onixd._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn onixd spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestOnixService = proxyquire('../../lib/services/onixd', {
				fs: {
			...
			onixd._loadSpawnConfiguration = sinon.stub();
			onixd.spawn = {};
			onixd.spawn.exec = 'onixd';
			onixd.spawn.datadir = '/tmp/onix';
			onixd.spawn.configPath = '/tmp/onix/onix.conf';
			onixd.spawn.config = {};
			onixd.spawnRestartTime = 1;
			onixd._loadTipFromNode = sinon.stub().callsArg(1);
			onixd._initZmqSubSocket = sinon.stub();
			onixd._checkReindex = sinon.stub().callsArg(1);
			onixd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			onixd._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(onixd, '_spawnChildProcess');
			onixd._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						onixd._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			onixd._spawnChildProcess(function(err) {
				...
				onixd._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				onixd.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn onixd spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestOnixService = proxyquire('../../lib/services/onixd', {
				fs: {
			...
			onixd._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						onixd._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			onixd.spawn.config.rpcpassword = 'password';
			onixd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			onixd._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			onixd._spawnChildProcess(function(err) {
				onixd._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			onixd._initZmqSubSocket = sinon.stub();
			onixd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			onixd._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var onixd = new OnixService(config);
			onixd.node.stopping = true;
			onixd.startRetryInterval = 100;
			onixd._loadTipFromNode = sinon.stub();
			onixd._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to onixd/);
				onixd._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			onixd.startRetryInterval = 1;
			var config = {};
			onixd._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				onixd._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			onixd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			onixd._connectProcess(config, function(err, node) {
				should.not.exist(err);
				onixd._loadTipFromNode.callCount.should.equal(1);
				onixd._initZmqSubSocket.callCount.should.equal(1);
				onixd._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd.options = {};
			onixd.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('Onix configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			onixd.options = {
				connect: [
					{}
				]
			...
			onixd.start(function(err) {
				onixd._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var onixd = new OnixService(baseConfig);
			var node = {};
			onixd._initChain = sinon.stub().callsArg(0);
			onixd._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			onixd.options = {
				spawn: {}
			};
			onixd.start(function(err) {
				should.not.exist(err);
				onixd.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(onixd, '_spawnChildProcess');
			var nodes = [{}];
			onixd._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			onixd.options = {
				connect: [
					{}
				]
			...
			onixd.start(function(err) {
				should.not.exist(err);
				onixd._initChain.callCount.should.equal(1);
				onixd._connectProcess.callCount.should.equal(1);
				onixd.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			onixd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				onixd.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			onixd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			onixd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			onixd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			onixd.syncPercentage(function(err) {
				should.exist(err);
				onixd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			onixd.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				onixd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var onixd = new OnixService(baseConfig);
			var args = onixd._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var onixd = new OnixService(baseConfig);
			var args = onixd._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var onixd = new OnixService(baseConfig);
			sinon.spy(onixd, '_normalizeAddressArg');
			onixd.nodes.push({
				client: {
			...
			var options = {};
			onixd.getAddressBalance(address, options, function(err) {
				onixd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			onixd.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				onixd.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					onixd._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			onixd.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			onixd.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = onixd._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = onixd._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = onixd._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = onixd._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				onixd._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			onixd.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(onixd, '_getHeightRangeQuery');
			sinon.spy(onixd, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			onixd.getAddressTxids(address, options, function(err) {
				should.exist(err);
				onixd._getHeightRangeQuery.callCount.should.equal(1);
				onixd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			onixd.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				onixd._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			onixd.getAddressTxids(address, options, function(err, txids) {
				...
				onixd.getAddressTxids(address, options, function(err, txids) {
					...
					onixd._getHeightRangeQuery.callCount.should.equal(2);
					onixd._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			onixd.getAddressTxids(address, options, function(err, txids) {
				...
				onixd.getAddressTxids(address, options, function(err, txids) {
					...
					onixd._getHeightRangeQuery.callCount.should.equal(4);
					onixd._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			onixd.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				onixd.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					onixd.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						onixd._getHeightRangeQuery.callCount.should.equal(3);
						onixd._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			var onixd = new OnixService(baseConfig);
			onixd.height = 10;
			var confirmations = onixd._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var onixd = new OnixService(baseConfig);
			var result = {};
			onixd._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var onixd = new OnixService(baseConfig);
			var result = {};
			onixd._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var onixd = new OnixService(baseConfig);
			var result = {};
			onixd._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
			var onixd = new OnixService(baseConfig);
			var result = {};
			onixd._getAddressDetailsForOutput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(onixd, '_getAddressDetailsForInput');
			sinon.spy(onixd, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = onixd._getAddressDetailsForTransaction(tx, addresses);
			...
			onixd._getAddressDetailsForInput.callCount.should.equal(1);
			onixd._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var onixd = new OnixService(baseConfig);
			sinon.spy(onixd, '_getConfirmationsDetail');
			onixd.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			onixd.height = 300;
			var addresses = {};
			onixd._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			onixd._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				onixd.getDetailedTransaction.callCount.should.equal(1);
				onixd._getAddressDetailsForTransaction.callCount.should.equal(1);
				onixd._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var onixd = new OnixService(baseConfig);
			onixd.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			onixd._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				onixd.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from onixcore addresses', function() {
			var addresses = [
				onixcore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				onixcore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var onixd = new OnixService(baseConfig);
			var strings = onixd._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var onixd = new OnixService(baseConfig);
			(function() {
				onixd._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var onixd = new OnixService(baseConfig);
			sinon.spy(onixd, '_normalizeAddressArg');
			onixd.maxAddressesQuery = 100;
			onixd.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				onixd._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var onixd = new OnixService(baseConfig);
			sinon.spy(onixd, '_normalizeAddressArg');
			sinon.spy(onixd, '_getAddressStrings');
			onixd.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				onixd._normalizeAddressArg.callCount.should.equal(1);
				onixd._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(onixd, '_normalizeAddressArg');
			sinon.spy(onixd, '_getAddressStrings');
			onixd.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			onixd.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				onixd._normalizeAddressArg.callCount.should.equal(1);
				onixd._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(onixd, '_normalizeAddressArg');
			sinon.spy(onixd, '_getAddressStrings');
			onixd.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			onixd._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			onixd.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				onixd._normalizeAddressArg.callCount.should.equal(1);
				onixd._getAddressStrings.callCount.should.equal(1);
				onixd.getAddressTxids.callCount.should.equal(1);
				onixd._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(onixd, '_normalizeAddressArg');
			sinon.spy(onixd, '_getAddressStrings');
			sinon.spy(onixd, 'getAddressTxids');
			onixd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			onixd.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				onixd._normalizeAddressArg.callCount.should.equal(1);
				onixd._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(onixd, '_normalizeAddressArg');
			sinon.spy(onixd, '_getAddressStrings');
			sinon.spy(onixd, '_paginateTxids');
			onixd._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(onixd, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			onixd.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			onixd.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				onixd._normalizeAddressArg.callCount.should.equal(1);
				onixd._getAddressStrings.callCount.should.equal(1);
				onixd.getAddressTxids.callCount.should.equal(1);
				onixd._getAddressDetailedTransaction.callCount.should.equal(2);
				onixd._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			onixd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			onixd.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			onixd.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			onixd.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			onixd.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			onixd.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			onixd.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			onixd.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			onixd.getAddressSummary(address, options, function(err, summary) {
				onixd._paginateTxids.callCount.should.equal(1);
				onixd._normalizeAddressArg.callCount.should.equal(1);
				onixd.getAddressTxids.callCount.should.equal(1);
				onixd.getAddressBalance.callCount.should.equal(1);
				onixd._paginateTxids.args[0][1].should.equal(0);
				onixd._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			onixd.getAddressSummary(address, options, function(err, summary) {
				...
				onixd.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					onixd._paginateTxids.callCount.should.equal(0);
					onixd._normalizeAddressArg.callCount.should.equal(2);
					onixd.getAddressTxids.callCount.should.equal(1);
					onixd.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			onixd.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				onixd._paginateTxids.callCount.should.equal(1);
				onixd._normalizeAddressArg.callCount.should.equal(1);
				onixd.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			onixd._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			onixd.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			sinon.spy(onixd, '_tryAllClients');
			onixd.nodes.push({
				client: {
					getBlock: sinon.stub().callsArgWith(2, { code: -1, message: 'Test error' })
				}
			});
			onixd.getRawBlock(blockhash, function(err) {
				should.exist(err);
				onixd._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			onixd.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			onixd.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			onixd.getBlock(0, function(err) {
				onixd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as onixcore object from height', function(done) {
			var onixd = new OnixService(baseConfig);
			sinon.spy(onixd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			onixd.getBlock(0, function(err, block) {
				should.not.exist(err);
				onixd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(onixcore.Block);
				done();
			});
		});
		it('will getblock as onixcore object', function(done) {
			var onixd = new OnixService(baseConfig);
			sinon.spy(onixd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			onixd.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				onixd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			onixd.getBlock(hash, function(err, block) {
				should.not.exist(err);
				onixd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			onixd.getBlock(0, function(err, block) {
				...
				onixd.getBlock(0, function(err, block) {
					should.not.exist(err);
					onixd._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			onixd.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var onixd = new OnixService(baseConfig);
			sinon.spy(onixd, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			onixd.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			onixd.getBlockHeader(10, function(err) {
				onixd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			onixd.getBlockHeader(blockhash, function(err) {
				onixd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			onixd.getBlockHeader(0, function(err) {
				onixd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			onixd.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				onixd._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			onixd.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				onixd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			onixd._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				onixd._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			onixd._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			onixd._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var onixd = new OnixService(baseConfig);
			sinon.spy(onixd, '_maybeGetBlockHash');
			onixd._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			onixd.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				onixd._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			onixd.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				onixd.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					onixd._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			onixd.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			onixd.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = onixcore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			onixd.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			onixd.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			onixd.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = onixcore.Transaction();
			(function() {
				onixd.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			onixd.getRawTransaction('txid', function(err) {
				should.exist(err);
				onixd._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			onixd.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			onixd.getTransaction('txid', function(err) {
				should.exist(err);
				onixd._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			onixd.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				onixd._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(onixcore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			onixd.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				onixd._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			onixd.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			onixd.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			onixd.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			onixd.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			onixd.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		...
		it('will call client getInfo and give result', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd.node.getNetworkName = sinon.stub().returns('testnet');
			...
			onixd.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			onixd.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			onixd.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				onixd.getSubsidy.callCount.should.equal(1);
				onixd.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			onixd.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			onixd.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		...
		it('will exit spawned process', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd.spawn = {};
			onixd.spawn.process = new EventEmitter();
			onixd.spawn.process.kill = sinon.stub();
			onixd.stop(done);
			onixd.spawn.process.kill.callCount.should.equal(1);
			onixd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			onixd.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			onixd.spawn.process.kill.callCount.should.equal(1);
			onixd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			onixd.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var onixd = new OnixService(baseConfig);
			onixd.shutdownTimeout = 300;
			onixd.spawn = {};
			onixd.spawn.process = new EventEmitter();
			onixd.spawn.process.kill = sinon.stub();
			onixd.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			onixd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				onixd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var onixd = new OnixService(baseConfig);
			sinon.spy(onixd, '_normalizeAddressArg');
			sinon.spy(onixd, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			onixd.nodes.push({
			...
			onixd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				onixd._getBalanceFromMempool.callCount.should.equal(1);
				onixd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			onixd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				onixd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			onixd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				onixd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			onixd.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			onixd.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			onixd.getJsonBlock(0, function(err) {
				should.exist(err);
				onixd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			onixd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				onixd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			onixd.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				onixd._tryAllClients.callCount.should.equal(2);
				onixd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			onixd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				onixd._tryAllClients.callCount.should.equal(1);
				onixd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var onixd = new OnixService(baseConfig);
			var emitter = new EventEmitter();
			onixd.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(onixd.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var onixd = new OnixService(baseConfig);
			var emitter = new EventEmitter();
			onixd.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			onixd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			onixd.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			onixd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			onixd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			onixd.unsubscribeBalance(emitter1);
			onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			onixd.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			onixd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var onixd = new OnixService(baseConfig);
			...
			onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			onixd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			onixd.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var onixd = new OnixService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			onixd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(onixd, 'unsubscribeBalanceAll');
			onixd.unsubscribeBalance(emitter1);
			onixd.unsubscribeBalanceAll.callCount.should.equal(1);
			onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			onixd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			onixd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			onixd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			onixd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			onixd.unsubscribeBalanceAll(emitter1);
			onixd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			onixd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			onixd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(onixd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var onixd = new OnixService(baseConfig);
			sinon.spy(onixd, '_notifyBalanceSubscriber');
			onixd._getAddressesFromTransaction = sinon.stub().returns(addresses);
			onixd.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				onixd._notifyBalanceSubscriber.callCount.should.equal(1);
				onixd._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			onixd._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				onixd.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			onixd._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			onixd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			onixd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			onixd.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			onixd.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			onixd.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			onixd.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				onixd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			onixd.getDgpInfo(function(err, result) {
				should.exist(err);
				onixd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			onixd.getMiningInfo(function(err, result) {
				should.exist(err);
				onixd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingInfo', function() {
		it('will give rpc error', function(done) {
			...
			onixd.getStakingInfo(function(err, result) {
				should.exist(err);
				onixd._tryAllClients.callCount.should.equal(1);
