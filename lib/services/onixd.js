...
/**
 * Provides a friendly event driven API to onixd in Node.js. Manages starting and
 * stopping onixd as a child process for application support, as well as connecting
 * to multiple onixd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Onix(options) {
  if (!(this instanceof Onix)) {
    return new Onix(options);
  }
...
util.inherits(Onix, Service);

Onix.dependencies = [];

Onix.DEFAULT_MAX_TXIDS = 1000;
Onix.DEFAULT_MAX_HISTORY = 50;
Onix.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Onix.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Onix.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Onix.DEFAULT_SPAWN_RESTART_TIME = 5000;
Onix.DEFAULT_SPAWN_STOP_TIME = 10000;
Onix.DEFAULT_TRY_ALL_INTERVAL = 1000;
Onix.DEFAULT_REINDEX_INTERVAL = 10000;
Onix.DEFAULT_START_RETRY_INTERVAL = 5000;
Onix.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Onix.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Onix.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Onix.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Onix.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Onix.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Onix.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Onix.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Onix.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Onix.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Onix.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Onix.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Onix.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Onix.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Onix.prototype._initCaches = function() {
...
Onix.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Onix.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Onix.prototype.getPublishEvents = function() {
  return [
    {
...
Onix.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'onixd/' + name, 'total:', this.subscriptions[name].length);
};

Onix.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'onixd/' + name, 'total:', this.subscriptions[name].length);
};


Onix.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
Onix.prototype.unsubscribeBalance = function(emitter, addresses) {
...
Onix.prototype.unsubscribeBalanceAll = function(emitter) {
...
Onix.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'onixd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Onix.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'onixd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Onix.prototype.unsubscribeAddressAll = function(emitter) {
...
Onix.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Onix.DEFAULT_CONFIG_SETTINGS;
...
Onix.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
Onix.prototype._expandRelativeDatadir = function() {
...
Onix.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in onixd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in onixd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in onixd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './onix.conf');

  log.info('Using onix config file:', configPath);
...
Onix.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of onixcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of onixcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of onixcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to onixd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from onixd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from onixd. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that onixd is undergoing a reindex. ' +
             ...
             'of onixcore-node services will start.');
    node._reindex = true;
  }
};

Onix.prototype._resetCaches = function() {
...
Onix.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Onix.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Onix.prototype._initChain = function(callback) {
  var self = this;
...
Onix.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 5889
  };
  if (this.node.network === bitcore.Networks.testnet) {
    networkOptions.rpcport = 15889;
  }
  return networkOptions;
};

Onix.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/onix.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/onix.conf';
    }
  }
  return networkPath;
};

Onix.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Onix.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('onixd/hashblock', message.toString('hex'));
    }
  }

};

Onix.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Onix.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Onix Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Onix.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Onix.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Onix.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
Onix.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('onixd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Onix.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('onixd/rawtransaction', message.toString('hex'));
    }
...
Onix.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Onix.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Onix.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Onix.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Onix.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Onix Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Onix.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Onix.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Onix.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/onixd.pid';
...
Onix.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting onix process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Onix process unexpectedly exited with code:', code);
        log.warn('Restarting onix child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Onix process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn onixd.'));
      }
...
Onix.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to onixd.'));
    }
 ...
 */
Onix.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Onix configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Onix.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Onix.prototype.syncPercentage = function(callback) {
  var self = this;
...
Onix.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, onixcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Onix.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Onix.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Onix.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Onix.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Onix.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in onixcore is most recent to less recent
 ...
 * @param {Function} callback
 */
Onix.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Onix.prototype._getConfirmationsDetail = function(transaction) {
...
Onix.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Onix.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Onix.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A onixcoin transaction id
 * @param {Function} callback
 */
Onix.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Onix.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Onix.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Onix.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Onix.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Onix.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Onix.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Onix.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Onix.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
Onix.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Onix.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Onix.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
Onix.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Onix.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
Onix.prototype.estimateSmartFee = function (blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Onix.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Onix.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Onix.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Onixcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Onix.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Onix.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Onix.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Onix.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Onix.prototype.getInfo = function(callback) {
...
Onix.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Onix.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('onixd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('onixd process did not exit'));
      }
 ...
 * @param {Function} callback
 */
Onix.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Onix.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Onix.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
Onix.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
Onix.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
Onix.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
Onix.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
Onix.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
Onix.prototype.getStakingInfo = function(callback) {
    var self = this,
